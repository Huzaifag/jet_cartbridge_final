...
<div class="container py-4">
    ...
    <form action="{{ route('warehouse.orders.dispatch', $order->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <label for="delivery_person_id" class="form-label fw-bold">Assign Delivery Person (Optional)</label>
            <select id="delivery_person_id"
                    name="delivery_person_id"
                    class="form-control @error('delivery_person_id') is-invalid @enderror">
                <option value="">Select Delivery Person</option>
                @foreach($deliveryMen as $deliveryMan)
                    <option value="{{ $deliveryMan->id }}" {{ old('delivery_person_id') == $deliveryMan->id ? 'selected' : '' }}>
