            ...
            <div class="user-dropdown">
                <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="userDropdown"
                    ...
                    <div class="avatar bg-primary text-white rounded-circle me-2">
                        {{ collect(explode(' ', Auth::user()->warehouse->name))->map(fn($w) => strtoupper(Str::substr($w, 0, 1)))->implode('') }}
                    </div>
                    <div class="d-none d-md-block">
                        <div class="fw-bold">{{ Auth::user()->warehouse->name }}</div>
                        <div class="small text-muted">Accountant Account</div>
                    </div>
                </a>
                <ul class="dropdown-menu dropdown-menu-end shadow" aria-labelledby="userDropdown">
                    <li class="dropdown-header">
                        <h6 class="mb-0">{{ Auth::user()->warehouse->name }}</h6>
                        <small class="text-muted">Warehouse Manager Account</small>
                    </li>
                    <li>
                        <hr class="dropdown-divider">
                    </li>
