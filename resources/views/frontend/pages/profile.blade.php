            ...
            <div class="row align-items-center">
                <div class="col-md-3">
                    ...
                    <div class="avatar-wrapper mb-3 mx-auto" style="width: 100px;">
                        ...
                        @else
                            <div
                                class="rounded-circle d-inline-flex align-items-center justify-content-center profile-initials">
                                {{ auth()->check() && auth()->user()->name ? strtoupper(substr(auth()->user()->name, 0, 1)) : '?' }}
                            </div>
                        @endif
                    </div>
                </div>
                <div class="col-md-9 text-md-start">
                    <h2 class="profile-title mb-1">{{ auth()->check() && auth()->user()->name ? auth()->user()->name : 'User' }}</h2>
                    <p class="profile-subtitle">{{ auth()->check() && auth()->user()->email ? auth()->user()->email : '' }}</p>
                    <p class="mt-3 text-dim">Welcome to your personal dashboard. Track your activity, manage your products,
                        and see your contribution impact.</p>
                </div>
            </div>
        </div>
                    ...
                    <div class="metric-box">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="metric-value"
                                style="color: var(--google-green);">{{ $userReviews->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-center pt-4 pb-2" style="height: 80px;">
                            <i class="fas fa-quote-right"
                                style="font-size: 3rem; color: var(--google-green); opacity: 0.2;"></i>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6 col-lg-3">
                    <div class="metric-box">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="metric-value"
                                style="color: var(--google-red);">{{ $followedSellers->count() + $followedManufacturers->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-center pt-4 pb-2" style="height: 80px;">
                            <i class="fas fa-store-alt"
                                style="font-size: 3rem; color: var(--google-red); opacity: 0.2;"></i>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6 col-lg-3">
                    <div class="metric-box coins-box">
                        ...
                        <div class="progress-ring-container mt-3"
                            style="background: radial-gradient(closest-side, white 70%, transparent 80% 100%), conic-gradient(var(--google-yellow) 55%, var(--google-border) 0);">
                            55%
                        </div>
                ...
                <div>
                    @if($purchasedProducts->count() > 0)
                            ...
                            /style> @foreach($purchasedProducts->take(6) as $product)
                                @if($product)
                                    <a href="#" class="text-decoration-none d-block product-card">
                                        @php
                                            $images = isset($product->images) ? (is_array($product->images) ? $product->images : json_decode($product->images, true)) : [];
                                            $firstImage = str_replace('\\/', '/', $images[0] ?? 'https://placehold.co/150x100/f1f3f4/5f6368?text=Product');
                                        @endphp
                                        <img src="{{ asset($firstImage) }}"
                                            onerror="this.onerror=null;this.src='https://placehold.co/150x100/f1f3f4/5f6368?text=Product';"
                                            class="product-image" alt="{{ $product->name ?? 'Product' }}">
                                        <div class="product-info">
                                            <h6 class="mb-1" style="font-size: 0.8rem; font-weight: 500;">
                                                {{ isset($product->name) ? Str::limit($product->name, 20) : 'Product' }}</h6>
                                            <small
                                                style="color: var(--google-blue); font-weight: 600;">${{ isset($product->b2c_price) ? number_format($product->b2c_price, 2) : '0.00' }}</small>
                                        </div>
                                    </a>
                                @endif
                            @endforeach
                        ...
                        @if($purchasedProducts->count() > 6)
                            <div class="text-center mt-3">
                                <small class="text-dim">Viewing {{ min(6, $purchasedProducts->count()) }} of
                                    {{ $purchasedProducts->count() }}</small>
                            </div>
                        @endif
                    @else
                        <div class="text-center py-5">
                            ...
                            <a href="{{ route('home') }}" class="btn-sm btn-primary"
                                style="background-color: var(--google-blue); color: white; border-radius: 4px; padding: 6px 12px; text-decoration: none;">Explore
                                Products</a>
                        </div>
                    @endif
                </div>
                                    ...
                                    <div class="d-flex align-items-center follow-item">
                                        <div class="flex-shrink-0 me-3">
                                            @if(isset($seller->logo) && $seller->logo)
                                                <img src="{{ asset('storage/' . $seller->logo) }}" alt="Seller Logo"
                                                    class="rounded-circle follow-avatar">
                                            @else
                                                <div class="rounded-circle d-inline-flex align-items-center justify-content-center follow-avatar"
                                                    style="background-color: var(--google-blue-light); color: var(--google-blue);">
                                                    <i class="fas fa-store" style="font-size: 0.8rem;"></i>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="flex-grow-1">
                                            <h6 class="mb-0" style="font-size: 0.95rem;">{{ isset($seller->business_name) ? $seller->business_name : 'Seller' }}</h6>
                                            <small class="text-dim">{{ isset($seller->products) ? $seller->products->count() : 0 }} products</small>
                                        </div>
                                        <span class="badge"
                                            style="background-color: var(--google-blue-light); color: var(--google-blue); font-weight: 500;">Seller</span>
                                    </div>
                                @endif
                            @endforeach
                                    ...
                                    <div class="d-flex align-items-center follow-item">
                                        <div class="flex-shrink-0 me-3">
                                            @if(isset($manufacturer->logo) && $manufacturer->logo)
                                                <img src="{{ asset('storage/' . $manufacturer->logo) }}" alt="Manufacturer Logo"
                                                    class="rounded-circle follow-avatar">
                                            @else
                                                <div class="rounded-circle d-inline-flex align-items-center justify-content-center follow-avatar"
                                                    style="background-color: var(--google-blue-light); color: var(--google-blue);">
                                        ...
                                        <div class="flex-grow-1">
                                            <h6 class="mb-0" style="font-size: 0.95rem;">{{ isset($manufacturer->business_name) ? $manufacturer->business_name : 'Brand' }}</h6>
                                            <small class="text-dim">{{ isset($manufacturer->products) ? $manufacturer->products->count() : 0 }} products</small>
                                        </div>
                                        <span class="badge"
                                            style="background-color: var(--google-blue-light); color: var(--google-blue); font-weight: 500;">Brand</span>
                                    </div>
                                @endif
                            @endforeach
                        </div>
                    @else
                        <div class="text-center py-5">
                            ...
                            <a href="{{ route('home') }}" class="btn-sm btn-secondary"
                                style="border: 1px solid var(--google-blue); color: var(--google-blue); border-radius: 4px; padding: 6px 12px; text-decoration: none;">Find
                                Brands</a>
                        </div>
                    @endif
                </div>
            ...
            <div>
                    ...
                    <div class="review-grid">
                        @foreach($userReviews->take(6) as $review)
                            <div class="review-card">
                                ...
                                <h6 class="mb-1" style="font-size: 1rem; color: var(--google-blue);">
                                    @if(isset($review->product) && $review->product)
                                        {{ isset($review->product->name) ? Str::limit($review->product->name, 40) : 'Product' }}
                                    @else
                                        <span class="text-dim">Product unavailable</span>
                                    @endif
                                </h6>
                                    ...
                                    <div class="mt-2">
                                        @if(count($mediaUrls) > 0)
                                            <small style="color: var(--google-green); font-weight: 500;">
                                                ...
                                                {{ count($mediaUrls) }}
                                                {{ $review->review_type == 'video' ? 'Video' : 'Image' }}{{ count($mediaUrls) > 1 ? 's' : '' }}
                                                Attached
                                            </small>
                                        @endif
                                    </div>
                ...
                @else
                    <div class="text-center py-5">
                        ...
                        <a href="{{ route('home') }}" class="btn-sm btn-primary"
                            style="background-color: var(--google-green); color: white; border-radius: 4px; padding: 6px 12px; text-decoration: none;">Contribute
                            Review</a>
                    </div>
                @endif
            </div>
