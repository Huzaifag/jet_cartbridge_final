...
<div class="container-fluid py-4">
    <header class="mb-4 pb-3 border-bottom">
        <h1 class="fw-bolder mb-0 text-dark">Salesman Dashboard</h1>
        <p class="text-muted mt-1">Track and manage customer orders and sales performance.</p>
    </header>
    ...
    <div class="row g-4 mb-5">
        ...
        <div class="col-md-3">
            <div class="stat-card bg-success-subtle border border-success">
                <div class="stat-label text-success">Confirmed Orders</div>
                <div class="stat-value text-success">{{ $stats['confirmed_orders'] }}</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card bg-warning-subtle border border-warning">
                <div class="stat-label text-warning">Pending Orders</div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white fw-bold py-3 border-bottom d-flex justify-content-between align-items-center">
            Recent Orders
            <a href="{{ route('salesman.placed-orders.index') }}" class="btn btn-primary btn-sm">
                ...
                <tbody>
                    @forelse ($recentOrders as $order)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('salesman.placed-orders.show', $order) }}" class="btn btn-sm btn-outline-primary">
                                    <i class="fas fa-eye"></i> View
                                </a>
                                @if($order->status === 'order_placed')
                                    <form action="{{ route('salesman.orders.confirm', $order) }}" method="POST" class="d-inline">
                                        @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-4">
                                <i class="fas fa-shopping-cart me-2 text-muted"></i> No orders yet.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
