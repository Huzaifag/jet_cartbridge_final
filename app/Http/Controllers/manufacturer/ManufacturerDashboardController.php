...
class ManufacturerDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Total Sales
        $totalSales = Order::where('manufacturer_id', $manufacturer->id)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('payment_status', 'paid')
            ->sum('total');

        // Total Orders
        $totalOrders = Order::where('manufacturer_id', $manufacturer->id)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->count();
        ...
        $newCustomers = Order::where('manufacturer_id', $manufacturer->id)
            ->whereBetween('created_at', [$startDate, $endDate])
        ...
        $salesChange = $this->calculateChange(
            Order::where('manufacturer_id', $manufacturer->id)
        ...
        $productsChange = $this->calculateChange(
            Product::where('manufacturer_id', $manufacturer->id)
        ...
        $topProducts = DB::table('order_items')
            ...
            ->where('orders.manufacturer_id', $manufacturer->id)
        ...
        // Recent orders
        $recentOrders = Order::where('manufacturer_id', $manufacturer->id)
            ->with('customer')
        ...
        // Recent reviews
        $recentReviews = Review::whereHas('product', function ($q) use ($manufacturer) {
        ...
        // Low stock products
        $lowStockProducts = Product::where('manufacturer_id', $manufacturer->id)
        ...
        $salesData = $this->getSalesData($manufacturer->id);

        // Distribution data
        $distributionData = $this->getDistributionData($manufacturer->id, $startDate, $endDate);

        // Revenue trend data (quarterly)
        $revenueData = $this->getRevenueData($manufacturer->id);

        return view('manufacturer.dashboard.index', compact(
            'totalSales',
            'totalOrders',
            'activeProducts',
            'newCustomers',
    ...
    private function getSalesData($manufacturerId)
    {
        $data = [];
        for ($i = 11; $i >= 0; $i--) {
            $date = Carbon::now()->subMonths($i);
            $sales = Order::where('manufacturer_id', $manufacturerId)
    ...
    private function getDistributionData($manufacturerId, $startDate, $endDate)
    {
    ...
    private function getRevenueData($manufacturerId)
    {
        $data = [];
        for ($i = 3; $i >= 0; $i--) {
            ...
            $revenue = Order::where('manufacturer_id', $manufacturerId)
